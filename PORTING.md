...
changed at all). The `controlknobs` revision — #435, four revisions back — found seven from a
package that was swept, whose commits were all correctly assessed, and whose *contents* nobody had
opened. **A swept package is not a read package, and a read commit log is not a read package
either.**
