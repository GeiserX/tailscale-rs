...
Crates that communicate with Tailscale's control plane (or a self-hosted control plane) and provide configuration for the data plane. The control plane handles authentication/authorization, node registration, policy updates, network map distribution, and much more for the nodes in a tailnet.
