...
impl PeerTracker {
    ...
    /// which must apply to the cached peers the same pass the netmap they were cached from already
    /// went through — Go replays its cached map through `setNetMapLocked`, so it runs this very
    /// filter.
