...
impl ControlRunner {
    ...
    fn maybe_sync_tka(&mut self, tka: &TkaStatus, self_ref: ActorRef<Self>) {
        if !tka.is_enabled() {
            ...
            if self.tka_synced.is_some() {
                ...
                // Drop the chain we were enforcing under: it describes a lock that is now off. This
                // fires on the transition only, so a chain left on disk by an *earlier* session is
                // not this call's business — `load_cached_netmap` removes that one at the next cold
                // start, when it can see that the cached frame has no use for it.
    ...
    ///
    /// The thin actor wrapper: it reads the two pieces of actor state the decision needs — the
    /// configured cache and the `cache-network-maps` grant on the last self node control sent — and
    /// hands them to [`persist_tka_chain_to`], which holds the decision itself and is tested
    /// directly. The grant comes from the same source `NetmapCache::observe` reads it from, so the
    /// chain is written exactly where the netmap is and removed exactly where the netmap is.
    async fn persist_tka_chain(&self, synced: &crate::tka_sync::SyncedTka) {
        ...
        persist_tka_chain_to(
            &cache,
            caching_granted,
            crate::tka_sync::encode_chain(&synced.store, synced.oldest),
        )
        .await;
    }
    ...
    /// is never *trusted* out of existence — the replay checks any chain it finds against the cached
    /// frame's own recorded head.
...
/// disk; the peers holding a valid signature survive and are dialed. This port persists the same
...
///
/// **Caveat, until `key_signature` decodes correctly off the wire:** a cached frame's `KeySignature`
/// is deserialized into `ts_control_serde::MarshaledSignature` (`&[u8]`), and serde_json hands a
/// `&[u8]` visitor the *raw bytes of the JSON string* rather than base64-decoding it the way Go
/// marshals `tkatype.MarshaledSignature`. So no real peer signature survives the netmap decode, the
/// filter's **admit** arm cannot fire against a real control plane, and a locked tailnet still
/// replays no peers end to end. Fixing that decode is a separate change; the filter here is what
/// makes the parity win land the moment it does.
///
...
/// Write (or remove) the Tailnet-Lock chain a cold start vouches with — the decision half of
/// [`ControlRunner::persist_tka_chain`], with no actor state in it so it is directly testable.
///
/// `caching_granted` is the `cache-network-maps` grant on the last self node control sent, and
/// `chain` is [`crate::tka_sync::encode_chain`]'s output for the freshly-synced store. There are
/// only two outcomes and no third: either the chain we now enforce is on disk, or nothing is.
///
/// * **grant present, chain encodable** ⇒ write it. That is the whole feature.
/// * **grant absent or overridden** ⇒ remove it. `NetmapCache::observe` discards the *netmap* on the
///   very same grant check, and the chain exists only to vouch for that netmap's peers; leaving it
///   would keep writing state for a node control has told to stop caching.
/// * **chain not encodable** (the store cannot be walked genesis→head) ⇒ remove it, rather than
///   leave an older chain that no longer describes what this node enforces.
pub(crate) async fn persist_tka_chain_to(
    cache: &ts_control::NetmapCache,
    caching_granted: bool,
    chain: Option<Vec<u8>>,
) {
    if !caching_granted {
...
/// the same pass the live netmap path runs, so a peer dialed from the cache is one the authority
/// authorized, while an unsigned peer, a peer whose signature fails, and a peer a newer rotation
/// obsoletes are dropped.
///
...
///   only its chonk.
///
/// This is also where a chain that has outlived its usefulness is deleted: a chain the cached frame
/// has no use for (the frame says the lock was off, the frame is gone, the chain does not verify, or
/// the chain is at another head) is removed here rather than left on disk forever. That covers the
/// case the enforcement paths cannot: a lock disabled while this node was *off*, whose first netmap
/// of the next session never crosses an enable→disable transition in [`ControlRunner`].
pub(crate) async fn load_cached_netmap(cache: &ts_control::NetmapCache) -> Option<StateUpdate> {
    let blob = cache.load_tka_chain().await;
    let had_chain = blob.is_some();
    let authority =
        blob.and_then(
            |blob| match crate::tka_sync::authority_from_encoded_chain(&blob) {
    ...
    let mut chain_was_used = false;
    let update = cache
        .load_state_update_vouched(|tka, peers| {
            let verdict = vouch_cached_peers(authority.as_ref(), tka, peers);
            chain_was_used = verdict.chain_describes_frame;
            verdict.replay
        })
        .await;

    if had_chain && !chain_was_used {
        cache.discard_tka_chain().await;
    }
...
/// What [`vouch_cached_peers`] decided about a netmap cached under an active Tailnet Lock.
pub(crate) struct CachedPeerVerdict {
    /// The peers this cold start may replay and dial.
    pub replay: Vec<Node>,
    /// Whether the persisted chain is the one this cached frame was taken under — i.e. whether it
    /// was of any use here. `false` means it vouched for nothing, and [`load_cached_netmap`] deletes
    /// it rather than leaving a chain no cold start can use.
    pub chain_describes_frame: bool,
}
...
///   describes a different lock says nothing about these peers. This is stricter than Go, which has
///   only its chonk and so nothing to reconcile; it costs peers in the window where the cached frame
///   is newer than the persisted chain, and it errs in the safe direction (an older chain cannot know
///   about a revocation the newer one carries).
///
...
pub(crate) fn vouch_cached_peers(
    ...
    let withhold = CachedPeerVerdict {
        replay: Vec::new(),
        chain_describes_frame: false,
    };
...
mod cached_replay_tests {
    ...
    //! dials before control has answered, and the chain on disk that decides it.
    //!
    ...
    use super::{TkaStatus, load_cached_netmap, persist_tka_chain_to, vouch_cached_peers};
    ...
    /// A locked tailnet: a genesis checkpoint trusting `signer` (signed by it, so the chain
    /// verifies), plus the store and the [`Authority`] a completed sync would hold.
    ...
    fn a_persisted_authority_replays_the_peers_it_authorizes() {
        ...
        let verdict = vouch_cached_peers(
            Some(&authority),
        ...
        assert!(
            verdict.chain_describes_frame,
            "the chain is at the head the cached frame recorded, so it describes this frame"
        );
        assert_eq!(
            verdict
                .replay
    ...
    fn without_an_authority_no_cached_peer_is_replayed() {
        ...
        let verdict = vouch_cached_peers(None, &cached_lock(&authority), vec![signed]);
        assert!(
            verdict.replay.is_empty(),
            "a peer nothing can vouch for waits for control's first netmap"
        );
        assert!(!verdict.chain_describes_frame);
    }

    /// A chain that is not at the head the cached frame recorded describes a different lock — a
    /// disable-and-re-enable under a fresh genesis, or a crash between writing the netmap and the
    /// sync that followed it. It vouches for nothing.
    #[test]
    fn a_chain_at_another_head_vouches_for_nothing() {
        ...
        let verdict = vouch_cached_peers(Some(&authority), &other, vec![signed.clone()]);
        assert!(verdict.replay.is_empty());
        assert!(!verdict.chain_describes_frame);
        ...
        for head in ["", "not base32"] {
            ...
            assert!(
                vouch_cached_peers(Some(&authority), &malformed, vec![signed.clone()])
                    .replay
                    .is_empty(),
    ...
    /// A scratch cache directory, private to this user so the cache's own vetting accepts it.
    #[cfg(unix)]
    fn scratch_cache(label: &str) -> (std::path::PathBuf, ts_control::NetmapCache) {
    ...
    /// Plant a cached netmap (raw `MapResponse` JSON, exactly what `NetmapCache` persists) in `dir`.
    #[cfg(unix)]
    fn plant_netmap(dir: &std::path::Path, tka_info: &str, peers: &str) {
        ...
        let path = dir.join(ts_control::NETMAP_CACHE_FILE);
    ...
    /// `TKAInfo` naming `authority`'s head, as control stamps it on a locked tailnet's netmap.
    fn tka_info_at(authority: &Authority) -> String {
        format!(
    ...
    // ===== the write half: what a completed sync leaves on disk =====

    /// The grant is present ⇒ the chain a cold start needs is on disk, byte for byte, and a cold
    /// start reads it back as the authority the sync held.
    #[cfg(unix)]
    #[tokio::test]
    async fn a_granted_node_persists_the_chain_it_enforces() {
        let signer = SigningKey::from_bytes(&[42u8; 32]);
        let (store, oldest, authority) = locked_tailnet(&signer);
        let blob = crate::tka_sync::encode_chain(&store, oldest).expect("encode");
        let (dir, cache) = scratch_cache("write-granted");

        persist_tka_chain_to(&cache, true, Some(blob.clone())).await;

        assert_eq!(
            cache.load_tka_chain().await.as_deref(),
            Some(&blob[..]),
            "a granted node must leave the chain a cold start vouches with"
        );
        assert!(
            crate::tka_sync::authority_from_encoded_chain(&blob)
                .expect("verifies")
                .head_matches(&authority.head()),
            "and it must be the chain of the authority this node enforces"
        );

        std::fs::remove_dir_all(&dir).ok();
    }

    /// The grant absent or overridden ⇒ the chain is **removed**, not written. `NetmapCache::observe`
    /// discards the netmap on this same check, and a chain for a netmap that is gone vouches for
    /// nothing; leaving one would keep writing state for a node told to stop caching.
    #[cfg(unix)]
    #[tokio::test]
    async fn a_node_told_to_stop_caching_loses_its_chain() {
        let signer = SigningKey::from_bytes(&[42u8; 32]);
        let (store, oldest, _authority) = locked_tailnet(&signer);
        let blob = crate::tka_sync::encode_chain(&store, oldest).expect("encode");
        let (dir, cache) = scratch_cache("write-withdrawn");

        persist_tka_chain_to(&cache, true, Some(blob.clone())).await;
        assert!(
            cache.load_tka_chain().await.is_some(),
            "control: it was written"
        );

        persist_tka_chain_to(&cache, false, Some(blob)).await;
        assert!(
            cache.load_tka_chain().await.is_none(),
            "withdrawing the grant must delete the chain, not leave it for the next cold start"
        );

        std::fs::remove_dir_all(&dir).ok();
    }

    /// A store that cannot be walked genesis→head encodes to nothing ⇒ the older chain is removed
    /// rather than left describing a lock this node no longer enforces.
    #[cfg(unix)]
    #[tokio::test]
    async fn an_unencodable_chain_removes_the_older_one() {
        let signer = SigningKey::from_bytes(&[42u8; 32]);
        let (store, oldest, _authority) = locked_tailnet(&signer);
        let blob = crate::tka_sync::encode_chain(&store, oldest).expect("encode");
        let (dir, cache) = scratch_cache("write-unwalkable");

        persist_tka_chain_to(&cache, true, Some(blob)).await;
        // What `encode_chain` returns for a store it cannot walk from the recorded genesis.
        persist_tka_chain_to(
            &cache,
            true,
            crate::tka_sync::encode_chain(&store, AumHash([0xEE; 32])),
        )
        .await;

        assert!(
            cache.load_tka_chain().await.is_none(),
            "a chain we cannot encode must not leave the previous one standing"
        );

        std::fs::remove_dir_all(&dir).ok();
    }

    // ===== the read half: what a cold start does with what is on disk =====

    /// End to end over the files a cold start actually finds: the chain the *write* half persisted is
    /// read back, re-verified, matched to the cached frame's head, and used to judge the cached peers
    /// — an unsigned one is dropped while the rest of the netmap replays. The chain stays on disk,
    /// because it is the one this frame was taken under.
    ///
    /// The *admit* side is proven on [`vouch_cached_peers`] directly rather than here: a cached
    /// frame's `KeySignature` currently decodes to the raw bytes of its JSON string (see
    /// [`super::replay_cached_netmap`]), so a real signature — 64 arbitrary bytes inside a CBOR
    /// structure — cannot be written into a JSON fixture at all.
    #[cfg(unix)]
    #[tokio::test]
    async fn a_cold_start_judges_the_cached_peers_against_the_persisted_chain() {
        let signer = SigningKey::from_bytes(&[42u8; 32]);
        let (store, oldest, authority) = locked_tailnet(&signer);
        let (dir, cache) = scratch_cache("with-chain");
        plant_netmap(&dir, &tka_info_at(&authority), UNSIGNED_CACHED_PEER);
        persist_tka_chain_to(&cache, true, crate::tka_sync::encode_chain(&store, oldest)).await;

        let replayed = load_cached_netmap(&cache)
            .await
        ...
        assert!(
            cache.load_tka_chain().await.is_some(),
            "the chain describes this frame, so it must survive for the next cold start too"
        );
    ...
    ///
    /// This is also the cross-session cleanup: a lock disabled while this node was off never crosses
    /// an enable→disable transition in the runner, so the chain left by the previous session is
    /// deleted here, where the cached frame shows it has no use.
    #[cfg(unix)]
    #[tokio::test]
    async fn a_cold_start_without_a_lock_replays_every_peer_and_drops_the_stale_chain() {
        let signer = SigningKey::from_bytes(&[42u8; 32]);
        let (store, oldest, _authority) = locked_tailnet(&signer);
        let (dir, cache) = scratch_cache("no-lock");
        plant_netmap(&dir, "", UNSIGNED_CACHED_PEER);
        persist_tka_chain_to(&cache, true, crate::tka_sync::encode_chain(&store, oldest)).await;
        ...
        assert!(
            cache.load_tka_chain().await.is_none(),
            "a chain the cached frame has no use for must not be left on disk for ever"
        );
    ...
    /// same outcome as never having persisted one — and the blob is deleted. The netmap replays.
    #[cfg(unix)]
    #[tokio::test]
    async fn a_cold_start_withholds_peers_when_the_persisted_chain_does_not_verify() {
        let signer = SigningKey::from_bytes(&[42u8; 32]);
        let (_store, _oldest, authority) = locked_tailnet(&signer);
        let tka_info = tka_info_at(&authority);

        // No chain at all.
        let (dir, cache) = scratch_cache("no-chain");
        plant_netmap(&dir, &tka_info, UNSIGNED_CACHED_PEER);
        let replayed = load_cached_netmap(&cache)
            .await
            .expect("the cached netmap replays");
        ...
        let (dir, cache) = scratch_cache("bad-chain");
        plant_netmap(&dir, &tka_info, UNSIGNED_CACHED_PEER);
        ...
        assert!(
            cache.load_tka_chain().await.is_none(),
            "a blob that is not a verified chain is never going to become one; drop it"
        );
        std::fs::remove_dir_all(&dir).ok();
    }

    /// A cached frame recording a head the persisted chain is not at: the peers are withheld and the
    /// chain — which describes some other lock — is dropped. This session's sync re-persists the one
    /// that matches.
    #[cfg(unix)]
    #[tokio::test]
    async fn a_cold_start_drops_a_chain_at_another_head() {
        let signer = SigningKey::from_bytes(&[42u8; 32]);
        let (store, oldest, _authority) = locked_tailnet(&signer);
        let (dir, cache) = scratch_cache("other-head");
        plant_netmap(
            &dir,
            &format!(
                r#""TKAInfo": {{ "Head": "{}", "Disabled": false }},"#,
                AumHash([0x5a; 32]).to_base32()
            ),
        ...
        assert!(
            cache.load_tka_chain().await.is_none(),
            "a chain describing another lock cannot vouch for this frame; drop it"
        );
