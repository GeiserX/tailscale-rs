...
//! binds `0.0.0.0:0` on the overlay netstack for both of its hops — the UDP query and the TCP hop
//! raced against it — never a host socket. A saturated server drops the flow (fail-closed).
...
mod tests {
    ...
    /// A filter compiled before the gate existed still reaches it. The peerAPI server task is
    /// spawned by `MagicDnsActor::on_start`, which runs on its own task and can therefore attach at
    /// any point relative to the first netmap; a cell written by the updater is readable whenever
    /// the reader shows up, so "the gate started late" cannot turn into "the gate refuses
    /// everyone".
    #[tokio::test]
    async fn a_gate_that_attaches_after_the_first_netmap_still_reads_the_filter() {
        ...
        // The production shape first: `Runtime::spawn` builds the receiver *before* the updater
        // spawns and hands that one to `MagicDnsActor`, so this is the receiver the gate really
        // reads.
        assert!(
            gate_says_yes(&v, &filter_rx, "100.64.0.1"),
        ...
        // And a receiver created *now* — a peerAPI server task that started after that netmap —
        // reads the filter that was compiled before it existed.
    ...
    ///
    /// The old shape applied the filter *inside* a bus handler, which is the shape `MagicDnsActor`
    /// has: it parks in handlers — its `Query` handler awaits a DNS forward for up to five seconds
    /// — and while it is parked, its idea of the filter cannot move. That is enough on its own to
    /// leave the gate refusing a peer control has granted, whether the newer filter was queued
    /// behind the park or skipped outright by best-effort delivery.
    ///
    /// So this test parks a subscriber in its handler on the first of three policies and then
    /// contrasts two answers from the *same* production `dns_source_allowed`: one against the cell
    /// the updater writes, one against the newest filter that parked subscriber actually holds.
    /// They differ, and that difference is the defect.
    #[tokio::test]
    async fn a_parked_bus_subscriber_goes_stale_while_the_cell_does_not() {
        ...
        // A subscriber that parks in its first handler. The mailbox is the smallest there is, so
        // the two publishes behind the park are queued at most one deep and best-effort delivery
        // may skip the rest outright — either way this subscriber's filter stops moving.
        ...
        // The tap is registered before any of the three, so the first one reaches it — and its
        // handler then parks, so the count cannot rise past one while `release_tx` is unset. That
        // is what the contrast below rests on: the subscriber is stuck on policy #1, and it is
        // stuck there whether the bus dropped the later two or merely queued them.
        ...
        // And the same gate, asked against the newest filter that subscriber actually holds — the
        // answer the peerAPI DoH gate gave while it applied the filter inside a bus handler —
        // refuses the peer control has granted. The two answers differ, which is the whole defect.
    ...
    /// The newest filter a bus subscriber actually received — the value a gate fed from a bus
    /// subscription would be answering from.
